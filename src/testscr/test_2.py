...
python test_2.py ../data/swegen_20161223/SNP_VCF.gz ../data/BAV_HiCap/BAV_P-E_InteractionDataset.txt  Lowfreq  --output ../results/Interactor_variant/Lowfreq_P_E.txt
python test_2.py ../data/swegen_20161223/SNP_VCF.gz ../data/BAV_HiCap/BAV_P-E_InteractionDataset.txt  Common  --output ../results/Interactor_variant/Common_P_E.txt
            ...
            for rec in Vcfin.fetch (chr[0], int(chr[1]), int(chr[2])):
                for i in range(0, len(rec.info["AC"])):
                    ...
                    if allecount(rec.info["AC"][i]) == "Rare" and args.freq == "Rare":
                        ...
                        result_values.append([rec.chrom,rec.stop,rec.alleles,(rec.info["AC"])[i]])
                        result[result_keys] = result_values

                    elif allecount(rec.info["AC"][i]) == "Lowfreq" and args.freq == "Lowfreq":
                        result_keys = "\t".join(fields)
                        result_values = result.get(result_keys,[])
                        result_values.append([rec.chrom,rec.stop,rec.alleles,(rec.info["AC"])[i]])
                        result[result_keys] = result_values
